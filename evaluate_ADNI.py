...
def auto_find_test_dir() -> Path:
    roots = [
        BASE_DIR / "data" / "splits_adni",
        BASE_DIR / "data" / "splits",
        BASE_DIR / "splits_adni",
        BASE_DIR / "splits",
    ]
    candidates = []
    for root in roots:
        ...
        for p in root.rglob("validated"):
            if _looks_like_validated_splits(p):
                candidates.append(p)
    if not candidates:
        raise SystemExit("No validated test.csv found under splits_adni/splits/**/validated")
...
def main():
    ...
    out_json = Path(args.results_dir) / "eval_summary.json"
