...
@dataclass
class ModelConfig:
    backbone: str = "r3d_18"  # r3d_18, r2plus1d_18 (2D + 1D temporal), custom_small imported (last resort and unsure how to apply it)
    pretrained: bool = False    # whether to load 400 video weights. = transfer learning strategy
    in_channels: int = 1        # 1 input channel due to grayscale MRI inputs
    out_dim: int = 3            # 3 output classes
    dropout: float = 0.2        # applied before final classification to reduce overfitting
...
# this works together to swap layer inside nested model with dotted name string.
# goes down through all to find parent module and uses the setattr to replace target child with new module.

# --------------------------------------------------------------------------------------------------------------

# this adapts pretrained c=3 to c=1 due to c=1 MRI input
...
def _replace_first_conv3d(model: nn.Module, in_channels: int, pretrained: bool) -> None:
    ...
    if first_conv.in_channels == in_channels:          # same input for input channels
        return
    ...
    new_conv = nn.Conv3d(
        ...
        padding=first_conv.padding,                                      # same settings
