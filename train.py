...
Trains ResNet3D-34 model for binary classification (CN vs VeryMild) on OASIS dataset.
This script trains for 100 epochs and saves the best model based on validation accuracy.

Configuration:
    - Task: Binary classification (CN vs Very Mild Impairment)
    ...
    - Class weights: [1.0, 1.7] (upweight minority class)

Output:
    - Saves best model to: results/best_model_binary_128.pth
    - Model is saved when validation accuracy improves
...
"""
    Train model for one epoch on training data.
...
"""
    Validate model on validation data (no training).
...
"""
    Main training loop - sets up data, model, and trains for 100 epochs.

    Trains ResNet3D-34 on OASIS dataset with:
        - Binary classification (CN vs VeryMild)
        - 128³ resolution MRI scans
        - Random flip augmentation on training set
