...
def build_argparser():
    ...
    ap.add_argument("--backbone", default="r3d_18", choices=["r3d_18", "r2plus1d_18", "custom_small"])
...
def main():
    ...
    if use_pretrained and int(args.freeze_epochs) > 0:
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if use_pretrained and int(args.freeze_epochs) > 0 and epoch == int(args.freeze_epochs) + 1:
            unfreeze_all(model)
            optimizer = torch.optim.AdamW(
                model.parameters(),
