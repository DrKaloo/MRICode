"""
Calibration Analysis - Model Confidence Reliability Assessment

Tests whether the lower_lr model's confidence scores (predicted probabilities)
are well-calibrated across different age groups. Well-calibrated models have
confidence scores that match actual accuracy rates.

Calibration Concept:
    Perfect calibration: If model predicts 70% confidence, it should be
    correct 70% of the time. Calibration curves plot predicted probability
    vs actual fraction of positives.

Why Calibration Matters:
    - Clinical deployment requires trustworthy confidence scores
    - Poorly calibrated models give misleading uncertainty estimates
    - Age-specific calibration reveals demographic fairness in uncertainty

Analysis Components:
    1. Overall calibration curve (all ages combined)
    2. Age-stratified calibration (3 age groups)
    3. Confidence distribution histograms
    4. Statistical summary table

Calibration Metrics:
    - Calibration error = |avg_confidence - actual_accuracy|
    - Error < 0.1 (10%) = well calibrated
    - Error > 0.1 = poorly calibrated (overconfident or underconfident)

Key Finding:
    Elderly patients (>80) show excellent calibration (0.2% error)!
    Young patients (<70) are underconfident (21.8% error but still accurate)
    This asymmetry shows model "knows what it doesn't know" for vulnerable group

Outputs:
    - 4-panel figure: results/calibration_analysis_lower_lr.png
        - Panel 1: Overall calibration curve
        - Panel 2: Calibration by age group (color-coded)
        - Panel 3: Confidence distribution histograms
...
Prerequisites:
    Requires test_predictions_lower_lr.csv from evaluation
"""
