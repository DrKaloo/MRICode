...
"""
Demographic Analysis of Model Predictions

Analyzes model performance across demographic subgroups to identify
potential biases and fairness issues in early Alzheimer's detection.

This script:
    - Loads predictions from lower_lr model
    - Calculates performance metrics stratified by age, sex, and education
    - Performs error analysis comparing correct vs incorrect predictions
    - Breaks down confusion matrix by demographic groups
    - Creates comprehensive visualization with 4-panel figure

Demographic Dimensions Analyzed:
    - Age: Three groups (<70, 70-80, >80 years)
    - Sex: Male vs Female
    - Education: Years of formal education

Metrics Computed:
    - Accuracy: Overall correctness
    - Sensitivity: Ability to detect VeryMild impairment (True Positive Rate)
    - Specificity: Ability to identify CN correctly (True Negative Rate)
    - Confusion matrix: TN, FP, FN, TP counts

Error Analysis:
    - Compares age, MMSE, sex, diagnosis between correct and incorrect predictions
    - Identifies systematic patterns in model failures

Key Finding:
    Lower_lr model shows 23.9% age gap (vs 42% baseline):
        - <70 years: 87.5% accuracy
        - >80 years: 63.6% accuracy
        - Gap = 23.9% (43% reduction from baseline!)

Outputs:
    - Prints: Detailed performance metrics and error analysis
    - Saves: results/demographic_analysis_lower_lr.png (4-panel figure)

Usage:
    python demographic_analysis.py

Prerequisites:
    Requires test_predictions_lower_lr.csv from evaluate_all_configs.py
"""

print("="*60)   #Load test data and predictions for demographic analysis
print("Loading Data")
print("="*60)
...
if has_predictions:
    ...
    print("\nDemographic analysis plot saved to: results/demographic_analysis_lower_lr.png")
