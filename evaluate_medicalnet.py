...
def main():
    ...
    # Detect MedicalNet vs torchvision checkpoint by inspecting the saved cfg keys.
    # MedicalNetConfig has "model_depth"; ModelConfig has "backbone".
    is_medicalnet = "model_depth" in cfg_d

    if is_medicalnet:
        med_cfg = MedicalNetConfig(
            model_depth=int(cfg_d.get("model_depth", 18)),
            shortcut_type=str(cfg_d.get("shortcut_type", "A")),
            pretrained_path=None,  # checkpoint weights override pretrained
            in_channels=int(cfg_d.get("in_channels", 1)),
            out_dim=num_classes,
            dropout=float(cfg_d.get("dropout", 0.2)),
            no_cuda=(device.type != "cuda"),
        )
        model = build_medicalnet(med_cfg)
    else:
        cfg = ModelConfig(
